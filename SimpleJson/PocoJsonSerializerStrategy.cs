...
namespace SimpleJson
{
    ...
    public class PocoJsonSerializerStrategy : IJsonSerializerStrategy
    {
        ...
        protected virtual void BuildMap(Type type, SafeDictionary<string, CacheResolver.MemberMap> memberMaps)
        {
            ...
            {
                memberMaps.Add(info2.Name, new CacheResolver.MemberMap(info2));
            }
        }
            ...
            else
            {
                if (value is IDictionary<string, object>)
                {
                    ...
                    if (ReflectionUtils.IsTypeDictionary(type))
                    {
                        ...
                        IDictionary newInstance = (IDictionary) CacheResolver.GetNewInstance(typeof(Dictionary<,>).MakeGenericType(new Type[] { type2, type3 }));
                    ...
                    SafeDictionary<string, CacheResolver.MemberMap> dictionary3 = this.CacheResolver.LoadMaps(type);
                    if (dictionary3 == null)
                    {
                        return value;
                    }
                    foreach (KeyValuePair<string, CacheResolver.MemberMap> pair2 in dictionary3)
                    {
                        CacheResolver.MemberMap map = pair2.Value;
                        if (map.Setter != null)
                        {
                ...
                if (ReflectionUtils.IsTypeGenericeCollectionInterface(type) || typeof(IList).IsAssignableFrom(type))
                {
                    ...
                    list2 = (IList) CacheResolver.GetNewInstance(typeof(List<>).MakeGenericType(new Type[] { type5 }));
        ...
        protected virtual bool TrySerializeUnknownTypes(object input, out object output)
        {
            ...
            foreach (KeyValuePair<string, CacheResolver.MemberMap> pair in this.CacheResolver.LoadMaps(type))
            {
