namespace Shadowsocks.View
{
    ...
    public class MenuViewController
    {
        ...
        private MenuItem CreateMenuItem(string text, EventHandler click,bool enabled = false)
        {
            return new MenuItem(I18N.GetString(text), click) {Checked = enabled};
        }
        ...
        private void LoadMenu()
        {
            this.contextMenu1 =
                new ContextMenu(new MenuItem[]
                {
                    ...
                    this.modeItem =
                        this.CreateMenuGroup("Mode",
                            new MenuItem[]
                            {
                                this.PACModeItem = this.CreateMenuItem("PAC", new EventHandler(this.PACModeItem_Click)),
                                this.globalModeItem =
                                    this.CreateMenuItem("Global", new EventHandler(this.GlobalModeItem_Click))
                            }),
                    this.ServersItem =
                        this.CreateMenuGroup("Servers",
                            new MenuItem[]
                            {
                                this.SeperatorItem = new MenuItem("-"),
                                this.ConfigItem =
                                    this.CreateMenuItem("Edit Servers...", new EventHandler(this.Config_Click)),
                                this.CreateMenuItem("Show QRCode...", new EventHandler(this.QRCodeItem_Click)),
                                this.CreateMenuItem("Scan QRCode from Screen...",
                                    new EventHandler(this.ScanQRCodeItem_Click))
                            }),
                    this.CreateMenuGroup("更新服务器",
                        new MenuItem[]
                        {
                            this.CreateMenuItem("立即更新", new EventHandler(this.UpdateServerNow)),
                            new MenuItem("-"),
                            this.CreateMenuItem("10分钟", new EventHandler(this.UpdateServer10)),
                            this.CreateMenuItem("20分钟", new EventHandler(this.UpdateServer20)),
                            this.CreateMenuItem("30分钟", new EventHandler(this.UpdateServer30)),
                            this.CreateMenuItem("40分钟", new EventHandler(this.UpdateServer40)),
                            this.CreateMenuItem("自动", new EventHandler(this.UpdateServerAutomatic),true)
                        }),
                    this.CreateMenuGroup("PAC ",
                        new MenuItem[]
                        {
                            this.localPACItem = this.CreateMenuItem("Local PAC", new EventHandler(this.LocalPACItem_Click)),
                            this.onlinePACItem =
                                this.CreateMenuItem("Online PAC", new EventHandler(this.OnlinePACItem_Click)),
                            new MenuItem("-"),
                            this.editLocalPACItem =
                                this.CreateMenuItem("Edit Local PAC File...",
                                    new EventHandler(this.EditPACFileItem_Click)),
                            this.updateFromGFWListItem =
                                this.CreateMenuItem("Update Local PAC from GFWList",
                                    new EventHandler(this.UpdatePACFromGFWListItem_Click)),
                            this.editGFWUserRuleItem =
                                this.CreateMenuItem("Edit User Rule for GFWList...",
                                    new EventHandler(this.EditUserRuleFileForGFWListItem_Click)),
                            this.editOnlinePACItem =
                                this.CreateMenuItem("Edit Online PAC URL...",
                                    new EventHandler(this.UpdateOnlinePACURLItem_Click))
                        }),
                    new MenuItem("-"),
                    this.AutoStartupItem =
                        this.CreateMenuItem("Start on Boot", new EventHandler(this.AutoStartupItem_Click)),
                    this.ShareOverLANItem =
                        this.CreateMenuItem("Allow Clients from LAN", new EventHandler(this.ShareOverLANItem_Click)),
                    new MenuItem("-"), this.CreateMenuItem("Show Logs...", new EventHandler(this.ShowLogItem_Click)),
                    this.CreateMenuItem("About...", new EventHandler(this.AboutItem_Click)), new MenuItem("-"),
                    this.CreateMenuItem("Quit", new EventHandler(this.Quit_Click))
                });
        }

        private void UpdateServerNow(object sender, EventArgs e)
        {
            AuthorityMannager.Update();
        }
        private void UpdateServer10(object sender, EventArgs e)
        {
            UpdateServerLate(sender,10);
        }
        private void UpdateServer20(object sender, EventArgs e)
        {
            UpdateServerLate(sender, 20);
        }
        private void UpdateServer30(object sender, EventArgs e)
        {
            UpdateServerLate(sender, 30);
        }
        private void UpdateServer40(object sender, EventArgs e)
        {
            UpdateServerLate(sender, 40);
        }

        private void UpdateServerAutomatic(object sender, EventArgs e)
        {
            UpdateServer(sender, () =>
            {
                TaskMannager.TaskMannager.IntervalTask.Stop();
                TaskMannager.TaskMannager.AutoTask.ReStart(15);
            });
        }

        private void UpdateServerLate(object sender, int min)
        {
            UpdateServer(sender, () =>
            {
                TaskMannager.TaskMannager.IntervalTask.ReStart(min * 60, min * 60);
            });
        }

        private void UpdateServer(object sender, Action hanlder)
        {
            var menu = sender as MenuItem;
            if (menu != null && !menu.Checked)
            {
                if (hanlder != null)
                {
                    hanlder();
